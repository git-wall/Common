package org.app.core.patterns.revisited.pipeline.v1;
...
public class Pipeline<T> {
    ...
    public Pipeline<T> ifEmptyThrow(String message) {
        if (value == null) {
            ...
            Verify.ifEmpty((String) value, message);
        } else if (value.getClass().isArray()) {
            Verify.ifEmpty((Object[]) value, message);
        } else if (value instanceof Map) {
            Verify.ifEmpty((Map<?, ?>) value, message);
        } else if (value instanceof Collection) {
            Verify.ifEmpty((Collection<?>) value, message);
        }
        return this;
    }
