...
public class Client4 {
    ...
    private ExchangeFilterFunction refreshTokenFilter(WebClient authWebClient) {
        ...
        return ExchangeFilterFunction
                .ofRequestProcessor(clientRequest ->
                    Mono.just(ClientRequestUtils.createWithAuth(clientRequest, accessToken))
                )
                .andThen((request, next) ->
                        next.exchange(request)
                            .flatMap(clientResponse -> retryIf401(authWebClient, request, next, clientResponse))
                );
    }
    ...
    private TokenInfo convertToTokenInfo(Map<String, Object> response) {
        ...
        if (info.expiresAt != null) {
            ...
            info.expiresAt = System.currentTimeMillis() + (info.expiresAt * 1000) - 5000L;
        }
