package org.app.concurrent.async;
    ...
    public void fireAndForget(Runnable task) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                log.warn("Fire-and-forget task failed", e);
            }
        }, executorService);
    }
    ...
    public synchronized CompletableFuture<T> execute(Supplier<T> task, T fallback) {
        ...
        if (circuitOpen) {
            ...
            if (currentTime - lastFailureTime >= resetMillis) {
                ...
                // The Circuit is still open, return fallback
...
class RateLimiter {
    ...
    private void refillPermits() {
        ...
        double newPermits = (now - lastRefillTime) / 1000.0 * permitsPerSecond;
